...
def inference(model, tokenizer, problem, num_beam=10, num_return_sequences=1):
    ...
    text = model.generate(**batch, **extra_details,
...
if __name__ == "__main__":
    ...
    project_name = f"{Path(args.model_path).name}-mbart-{args.dataset_name}-n{args.data_limit}-src{args.max_source_length}-tgt{args.max_target_length}"
